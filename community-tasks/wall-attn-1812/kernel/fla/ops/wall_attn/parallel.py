...
# Vendored for Ascend NPU (task #1812). Adaptations vs upstream:
#   - removed @fla.ops.backends.dispatch (GPU backend routing) and einops dependency;
#   - autotune spaces pruned to num_warps {2,4} x num_stages {2} to bound the NPU
#     compile/autotune sweep (issue acceptance item: 507014/507034 timeouts);
#   - kernel launches pass ascend_compile_kwargs() (disable auto-multi-buffer,
#     bounding UB usage); kernel bodies and numerics are unchanged.
...
def parallel_wall_attn_bwd(
    ...
    # GQA gradient reduction over the query-head group (was einops.reduce;
    # rewritten with native torch to drop the einops dependency).
    dk = dk.view(B, T, H, G, K).sum(dim=3)
    dv = dv.view(B, T, H, G, V).sum(dim=3)
